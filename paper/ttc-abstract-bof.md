...
**Evidence and intervention.** Monte Carlo simulations first tested whether the two-rate structure behaved as predicted. A completion guard raised high-complexity pass@1 by about +2 to +17 percentage points without automated QA but had near-zero effect under the full pipeline, matching the predicted ceiling; these remain simulation results. We then applied the framework to Modus Web Components. In the first medium run, the agent broke Escape-and-focus behavior, reported completion, failed QA, and never entered repair. After adding a pre-open evidence check and fixing the QA→repair handoff, a comparable run failed QA, was repaired, and passed the next QA round. Its provisional `V*` moved 0.56→0.74 using placeholder, not repository-fitted, weights. The number is directional; the stronger live result is the observed transition from an open cycle to a completed recovery cycle. This before/after case is the provocation for the room.
